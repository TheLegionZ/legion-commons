...
public class ParseableTime {
	...
	public ParseableTime(long time, TimeUnit timeUnit) {
		this(TimeUnit.MILLISECONDS.convert(time, timeUnit));
	}

	public static ParseableTime fromMilis(long milis) {
		return new ParseableTime(milis);
	}

	public static ParseableTime fromNow(long additionalTime, TimeUnit unit) {
		return new ParseableTime(System.currentTimeMillis() + (unit.toSeconds(additionalTime) + 1000));
	}

	private ParseableTime(long milliseconds) {
		if (milliseconds < 0) {
	...
	public static ParseableTime fromNow() {
		return new ParseableTime(System.currentTimeMillis());
	}

	public static ParseableTime parseString(String timeString) throws TimeParseException {
		if (timeString == null) {
		...
		return new ParseableTime(totalMilliseconds);
	}
	...
	private long appendTime(long time, long unitInMS, String name, StringBuilder builder) {
		...
		if (timeInUnits > 0) {
			builder.append(", ").append(timeInUnits).append(name);
		}
